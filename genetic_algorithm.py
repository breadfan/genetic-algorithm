...
class GeneticAlgorithm(list):
    ...
    def selection(self):
        ...
        if self.minimize:
            rand_cand_ids = rand_cand_ids[
                np.arange(self.n_pop), np.argmin(self.scores[rand_cand_ids], axis=1)]  # for minimization
        else:
            rand_cand_ids = rand_cand_ids[
                np.arange(self.n_pop), np.argmax(self.scores[rand_cand_ids], axis=1)]  # watching at k random candidates
    ...
    def crossover(self, parents):
        pairs_ids = np.stack([np.arange(0, self.n_pop - 1, 2), np.arange(1, self.n_pop, 2)],
                             axis=1)  # just matrix (n-1, 2) with pairs of indexes: [[0, 1], [2, 3], [4, 5]...]

        probs_of_selection = np.random.rand(self.n_pop // 2)  # probs of selection
        valid_pairs = pairs_ids[
            probs_of_selection <= self.crossover_prob]  # "<" because we're draw probability of "not to be crossovered"
